...
Keep in mind the instructions in the file, e.g. (/br)
This EUI must be in little-endian format, so least-significant-byte first. (/br)
When copying an EUI from The Things dashboard, this means to reverse the bytes. (/br)
For TTN issued EUIs the last bytes should be 0xD5, 0xB3,0x70. (/br)
